export class ChatRoom {
  ...
  async init() {
    ...
    try {
      this.state.storage.sql.exec("CREATE TABLE IF NOT EXISTS messages (id TEXT PRIMARY KEY, type TEXT NOT NULL DEFAULT 'message', username TEXT DEFAULT '', content TEXT DEFAULT '', timestamp INTEGER DEFAULT 0, imageKey TEXT DEFAULT '')");
      ...
      for (var r of this.state.storage.sql.exec("SELECT value FROM settings WHERE key = 'image_enabled'")) this.imageEnabled = r.value !== "false";
    } catch (e) {}
  }
  ...
  async fetch(request) {
    ...
    if (path === "/api/image-toggle") return await this.handleImageToggle(request);
    if (path === "/api/history") return this.json(this.getMessages());
    if (path === "/api/status") return this.json({ muted: this.isMuted, messageCount: this.getMessageCount(), onlineCount: this.clients.size, announcement: this.announcement, title: this.roomTitle, imageEnabled: this.imageEnabled });
  ...
  getMessages() {
    try {
      ...
      var res = [];
      for (var row of cur) res.push({ id: row.id, type: row.type, username: row.username, content: row.content, timestamp: row.timestamp, imageKey: row.imageKey || "" });
      return res;
    } catch (e) { return []; }
  }
  ...
  insertMessage(msg) {
    try {
      this.state.storage.sql.exec("INSERT INTO messages (id, type, username, content, imageKey, timestamp) VALUES (?1, ?2, ?3, ?4, ?5, ?6)", msg.id, msg.type, msg.username, msg.content, msg.imageKey || "", msg.timestamp);
  ...
  async handleImageToggle(request) {
    if (request.method === "GET") return this.json({ enabled: this.imageEnabled });
    var data = {};
    try { data = await request.json(); } catch (e) {}
    if (!this.verifyPassword(data.password)) return this.json({ success: false, error: "\u5BC6\u7801\u9519\u8BEF" });
    this.imageEnabled = typeof data.enabled === "boolean" ? data.enabled : !this.imageEnabled;
    try { this.state.storage.sql.exec("INSERT OR REPLACE INTO settings (key, value) VALUES ('image_enabled', ?1)", String(this.imageEnabled)); } catch (e) {}
    this.broadcast({ type: "image_toggle", enabled: this.imageEnabled });
    this.broadcast({ type: "system", content: this.imageEnabled ? "\u7BA1\u7406\u5458\u5DF2\u5F00\u542F\u56FE\u7247\u4E0A\u4F20" : "\u7BA1\u7406\u5458\u5DF2\u5173\u95ED\u56FE\u7247\u4E0A\u4F20", timestamp: Date.now() });
    return this.json({ success: true, enabled: this.imageEnabled });
  }
  ...
  handleSession(ws) {
    ...
    ws.send(JSON.stringify({ type: "image_toggle", enabled: this.imageEnabled }));
    ...
    ws.addEventListener("message", function(event) {
      try {
        ...
        if (data.type === "message" && (data.content || data.imageKey)) {
          if (data.content) data.content = data.content.trim();
          if (!data.content && !data.imageKey) return;
          ...
          ws.lastMsgTime = now;
          var msg = { id: crypto.randomUUID(), type: "message", username: ws.username, content: self.sanitize(data.content || ""), timestamp: now, imageKey: data.imageKey || "" };
